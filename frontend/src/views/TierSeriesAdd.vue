    ...
    methods: {
        ...
        async ValidateGlobalForm() {
            ...
            if (hasEmptyNames) {
                Swal.fire({
                    title: 'Erreur',
                    text: 'Tous les libellés des séries doivent être renseignés.',
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
                return;
            ...
            Swal.fire({
                title: 'Ajout',
                html: `Voulez-vous vraiment Ajouter ces Series `,
                icon: 'question',
        ...
        async submitForm() {
            try {
                ...
                const response = await axios.post('/api/tiers/series', payload, {
                    headers: {
...
<template>
    <Layout>
        <pageheader title="Ajouter Familles" secondaryPageHeader="Familles" pageTitle="Tiers" />
                ...
                <BCard no-body style="width: 60%; margin: auto;">
                    <BCardHeader>
                        <a-input-group compact style="margin: auto; width: 50%;"> 
                            <a-button type="primary" @click="addSeriesRows(lignes)">Ajouter </a-button>
                            <a-input-number v-model:value="lignes" min="1" max="100" :defaultValue="1" addonAfter="lignes"></a-input-number>
                        </a-input-group>
                    </BCardHeader>
                    <BCardBody>
                        ...
                        <a-table 
                            :dataSource="series" 
                            :columns="[
                                { title: 'Libellé', dataIndex: 'name', key: 'name' },
