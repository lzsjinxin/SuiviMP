    ...
    methods: {
        async ValidateGlobalForm() {
            ...
            if (!isFormValid) {
              ...
              Swal.fire({
                title: 'Ajout',
                html: `Voulez-vous vraiment Modifier cet Utilisateur `,
                icon: 'question',
        ...
        async submitForm(){
            ...
            const response = await axios.patch('/api/users/'+this.$route.params.id,payload,{
        headers: {
        ...
        fetchUser(){
            axios.get('/api/users/'+this.$route.params.id)
                .then(response => {
                    ...
                    this.selectedDepartment = response.data[0].department.id
                })
                .catch(error => {
                    console.error(error)
                })
...
<template>
    <Layout>
        <pageheader :title='"Modifier Utilisateur N°" + this.$route.params.id' pageTitle="Emplacements" />
