    ...
    methods: {
        async ValidateGlobalForm() {
            ...
            if (!isFormValid) {
              ...
              Swal.fire({
                title: 'Ajout',
                html: `Voulez-vous vraiment Ajouter une unité avec le libellé suivant : 
                <div class="row mt-3">
        ...
        async submitForm(){
            ...
            const response = await axios.post('/api/units/',payload,{
        headers: {
...
<template>
    <Layout>
        <pageheader title="Ajouter des Unités" pageTitle="Unités" />
